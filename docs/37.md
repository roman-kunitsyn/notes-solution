Next we create the production deployment runbook. It records the exact order of operations without selecting a VPS provider yet.

## 1. Create the document

```zsh
mkdir -p docs
nvim docs/production-deployment.md
```
...
git commit -m "Document production deployment procedure"
````

At this point, the manual production path is documented end to end. The next local milestone is CI/CD: first CI for migrations, database tests, shell syntax and Compose validation; then a deliberately separate deployment workflow that we will leave disabled until the VPS exists.
